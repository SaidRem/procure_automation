...
- **suppliers** — модель `Shop`, загрузка прайса по ссылке (только https,
  ...
  импорта, Celery-задача с политикой повторов и журнал запусков
  `ImportLog`: одна запись на запуск, счётчик попыток, коды ошибок и
  счётчики результата.
...
- API поставщика — создание магазина, запуск импорта, переключение приёма
  заказов, список своих заказов;
...
- админка каталога и заказов, экспорт прайса, Docker-образ приложения.

Импорт прайса пока запускается только из сервисного слоя или Celery:
endpoint'а и действия в админке для него ещё нет.
...
backend/
  ...
  users/       пользователи, аутентификация, контакты, админка
  suppliers/   поставщики, разбор прайса, журнал и задача импорта, админка
