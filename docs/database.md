...
Реализованы модельный, сервисный слои и API (миграция
orders/0001_initial). Админка заказов ещё не создана. Сервисный слой и
API схему не меняют: новых полей и миграций они не добавили.
...
- OrderItem (quantity, snapshot-поля — product_name, shop_name, price,
  ...
  Правило закреплено амендментами ADR-022 от 2026-08-30; на Order
  исключение не распространяется. Предусмотренная операция удаления —
  orders.services.remove_item; позиция с обнулённой ссылкой на
  предложение остаётся удаляемой, пока заказ в состоянии basket.
...
- Реализовано без постоянных моделей: задача отправки писем и публичный
  сервисный слой состояния не хранят, миграций у приложения нет.
  Решение может быть пересмотрено, если понадобится аудит отправленных
  писем.
- Перенос задачи из users выполнен (ADR-010, amendment от 2026-08-30):
  users/tasks.py и фасад users/services/notifications.py удалены,
  действует notifications.services. Схему БД перенос не затронул.
...
Реализация notifications соответствует этой схеме: приложение читает
orders.models (движение вниз по цепочке) для сборки писем по заказу —
ADR-005 относит подтверждение заказа и накладную администратору к его
функциям, а собрать письмо, не прочитав заказ, нельзя. Модели users,
catalog и suppliers оно не импортирует: получатель, адрес и состав
письма берутся из snapshot самого заказа (ADR-003, ADR-024, ADR-027).
Обратной зависимости нет — orders вызывает только
notifications.services. Правила проверяются тестом
notifications/tests/test_dependencies.py.